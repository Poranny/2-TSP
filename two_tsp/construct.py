...
from two_tsp.helpers import cycle_cost
...
def greedy_nearest_neighbor(distance_matrix: List[List[int]]) -> Tuple[List[int], List[int]]:
...
def greedy_cycle(distance_matrix: List[List[int]]) -> Tuple[List[int], List[int]]:
    ...
    if n == 0:
        return [], []
    verts = set(range(n))
    s1 = random.choice(tuple(verts)); verts.remove(s1)
    s1b = min(verts, key=lambda v: distance_matrix[s1][v]); verts.remove(s1b)
    cycle1 = [s1, s1b]
    if verts:
        s2 = max(verts, key=lambda v: distance_matrix[s1][v]); verts.remove(s2)
        if verts:
            s2b = min(verts, key=lambda v: distance_matrix[s2][v]); verts.remove(s2b)
            cycle2 = [s2, s2b]
        else:
            cycle2 = [s2]
    else:
...
def regret_cycle(distance_matrix: List[List[int]], weighted: bool, alpha: float = 0.75) -> Tuple[List[int], List[int]]:
    n = len(distance_matrix)
    if n == 0:
        return [], []
    verts = set(range(n))
    ...
    while verts:
        ...
        for v in verts:
            ...
            for i in range(m):
                j = (i+1) % m
                delta = (distance_matrix[current[i]][v] +
            ...
            regret = (alpha*(second-best_c) - (1-alpha)*best_c) if weighted else (second-best_c)
